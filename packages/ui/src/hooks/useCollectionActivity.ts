...
type CollectionActivityResponse =
  paths['/collections/{collection}/activity/v2']['get']['responses']['200']['schema']

type CollectionActivityQuery =
  paths['/collections/{collection}/activity/v2']['get']['parameters']['query']
  ...
  const { data, mutate, error, isValidating, size, setSize } =
    useSWRInfinite<CollectionActivityResponse>(
      (pageIndex, previousPageData) => {
        ...
        if (previousPageData && previousPageData.activities?.length === 0) {
          return null
